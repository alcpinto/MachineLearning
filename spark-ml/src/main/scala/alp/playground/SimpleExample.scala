...
object SimpleExample extends App {
    ...
    private val paramMap: ParamMap = ParamMap(lr.maxIter -> 20)
        .put(lr.maxIter, 30)  // Specify 1 Param. This overwrites the original maxIter.
        .put(lr.regParam -> 0.1, lr.threshold -> 0.55)  // Specify multiple Params.

    // One can also combine ParamMaps.
    private val paramMap2: ParamMap = ParamMap(lr.probabilityCol -> "myProbability")  // Change output column name.
    private val paramMapCombined: ParamMap = paramMap ++ paramMap2

    // Now learn a new model using the paramMapCombined parameters.
    // paramMapCombined overrides all parameters set earlier via lr.set* methods.
    private val model2 = lr.fit(training, paramMapCombined)
    ...
    private val transformed: DataFrame = model2.transform(test).cache()
